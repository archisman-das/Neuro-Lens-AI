...
# Llama 3.3 70B + Llama 3.2 90B Vision via $HF_TOKEN Space secret); if no
# token is set the dashboard falls back to the deterministic radiology report
# which is rich on its own and zero-hallucination by construction.
...
# scripts, or .pt checkpoints; only what the dashboard actually needs at
# request time. ONNX weights ride along (small enough; see .dockerignore).
...
ENV HF_MODEL_TEXT="meta-llama/Llama-3.3-70B-Instruct" \
    HF_MODEL_VISION="meta-llama/Llama-3.2-90B-Vision-Instruct"
